...
@foreach ( (isset($items) ? $items : array()) as $key => $value)
    <tr>
        ...
        <td>
            <h5><a href="/account/actions/{{ $value['id'] }}">{{ $value['course']['title'] ?? '-' }}</a></h5>
        </td>
        <td>
            {{ date('d.m.Y', strtotime($value['date_end'])) }}
        </td>
        <td>
            {{ $value['new_price'] }} ₽
        </td>
        ...
        <td>
            <a href="/account/actions/{{ $value['id'] }}" class="btn border-btn-dark btn-block btn-edit">
               <div>
