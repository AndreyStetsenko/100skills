      ...
      <div class="uk-clearfix mt-3">
          ...
          <div class="uk-float-right">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {{-- {{ date('Y-m-d H:i:s', $item->action->date_start)->diffInMinutes(date('Y-m-d H:i:s', $item->action->date_end)) }} --}}

                {{-- @if ( Carbon\Carbon::parse($item->action->date_end ?? null) < now() )
                  <span class="uk-text-danger">Акция закончилась</span>
                @else
                    <span class="uk-text-success">Акция активна</span>
                @endif --}}
          </div>
      </div>
